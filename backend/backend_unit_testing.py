...
def test_create_task_without_jwt():
    response = client.post("/tasks", json={"header": "TestTask", "text": "TestText", "status_id": "0"})
    assert response.status_code == 401

def test_create_task_with_invalid_jwt():
    response = client.post("/tasks", json={"header": "TestTask", "text": "TestText", "status_id": "0"}, headers={"Authorization": "Bearer INVALIDJWT"})
    assert response.status_code == 401
...
def test_authorization_invalid_password(test_db):
    ...
    test_db.commit()
    response = client.post("/login", data={"login_input": "UnitTestUser", "password_input": "123456"})
    assert response.status_code == 401

def test_register_with_taken_login(test_db):
    test_db.add(Users(id=0, login="UnitTestUser", password="VeryHardPassword", email="Test@gmail.com"))
    test_db.commit()
    response = client.post("/register", data={"login_input": "UnitTestUser", "password_input": "123456", "email_input": "CoolEmail@gmail.com"})
...
def test_register_with_taken_email(test_db):
    ...
    test_db.commit()
    response = client.post("/register", data={"login_input": "AnotherUser", "password_input": "123456", "email_input": "CoolEmail@gmail.com"})
    assert response.status_code == 409