...
9. Назначение ответственных за выполнение задачи (Можно назначить и себя). Задача отобразится у ответственного пользователя, но он не сможет ее редактировать.

У каждой задачи может быть название, текст, статус, создатель и ответственный. Уникальный ID, дата создания и создатель заполняются автоматически на сервере. Для указания ответственного нужно указать валидный логин пользователя.
